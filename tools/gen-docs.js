    ...
    definition: {
        ...
        components: {
            ...
            schemas: {
                ...
                Endroit: {
                    type: 'object',
                    description: "Un endroit réservable : salle de réunion, salle de montage, matériel…",
                    properties: {
                        id: { type: 'integer' },
                        libelle: { type: 'string' },
                        slug: { type: 'string' },
                        description: { type: 'string' },
                        type: { type: 'string', enum: ['reunion', 'montage', 'materiel', 'autre'] },
                        type_libelle: { type: 'string' },
                        ordre: { type: 'integer' },
                        id_equipe: { type: 'integer', description: "Équipe à laquelle la réservation est réservée (0 = ouvert à tous)" },
                        equipe: { type: 'string' },
                        ferme: { type: 'boolean' },
                        message_ferme: { type: 'string' },
                        lien: { type: 'string', nullable: true, description: 'Lien de visioconférence' },
                        coordonnees: { type: 'string', description: 'Position sur le plan des locaux (CSS)' },
                        notif: { type: 'boolean' },
                        emails_dest: { type: 'string' },
                        emails_notif: { type: 'array', items: { type: 'string' } },
                        ip: { type: 'string', description: "IP de l'écran posé devant l'endroit" },
                        modification: { type: 'string', format: 'date-time' },
                        ecran: { type: 'string', description: "URL de l'image d'écran générée par sogest" },
                        disponible: { type: 'boolean', description: 'Libre à cet instant' },
                    },
                },
                EndroitEcriture: {
                    type: 'object',
                    properties: {
                        ...
                        message_ferme: { type: 'string' },
                        notif: { type: 'boolean' },
                        emails_dest: { type: 'string', description: 'Adresses séparées par des virgules' },
                        lien: { type: 'string' },
                ...
                Reservation: {
                    ...
                    properties: {
                        ...
                        couleur1: { type: 'string', description: "Couleur de fond de l'utilisateur (identique à sogest)" },
                        couleur2: { type: 'string', enum: ['black', 'white'] },
